...
- FINDINGS, RESEARCH PROJECTS, FUTURE DIRECTIONS, and TECHNIQUES come primarily from the PI's PAPERS. Your FIRST move for findings is search_author_papers with the PI's name — it returns ~25 of their papers WITH abstracts in a single call, scoped to UCSD, and is the broadest, most reliable source (it disambiguates common names better than keyword search). Extract findings from those abstracts. Then use search_pubmed / fetch_full_paper to pull full-text Discussion + Future Directions from the 1-3 most relevant papers. Many UCSD faculty URLs are THIN institutional profile pages (profiles.ucsd.edu, department bios) that list paper titles but no actual findings — do NOT depend on the homepage for findings; the papers are the real, unbiased source. If the given page is thin or will not load, go straight to search_author_papers by the PI's name.
