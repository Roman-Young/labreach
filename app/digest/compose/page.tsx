...
function ExampleCard({ ex }: { ex: EmailExample }) {
  ...
  nonOverlap.forEach((m, i) => {
    ...
    nodes.push(
      <mark key={`m${i}`} className="bg-gold/20 text-ink rounded px-0.5">
        {m.quote}
        <sup className="text-[10px] font-semibold text-gold-dark ml-0.5">{m.num}</sup>
      </mark>,
    )
  ...
  return (
    <div className="rounded-lg border border-hairline bg-surface/50 p-4">
      <p className="text-[11px] uppercase tracking-[0.1em] text-muted-2 mb-2">{ex.scenario}</p>
      <p className="whitespace-pre-wrap text-[13px] text-ink-2 leading-relaxed font-serif">{nodes}</p>
      <div className="mt-3 border-t border-hairline pt-3 space-y-2">
        <p className="text-[11px] uppercase tracking-[0.1em] text-gold-dark font-medium">Why these lines work</p>
        {ex.annotations.map((a, i) => (
          <p key={i} className="text-[12.5px] text-muted leading-relaxed">
            <span className="font-semibold text-gold-dark">{i + 1}.</span> {a.note}
          </p>
        ))}
      </div>
    </div>
...
export default function ComposePage() {
  ...
  if (!hydrated) return <main className="max-w-3xl mx-auto px-4 py-10 text-sm text-muted-2">Loading…</main>
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between gap-3">
        ...
        <h1 className="text-lg font-semibold tracking-tight text-ink">Your email skeleton</h1>
      </div>

      <p className="mt-3 text-[13px] rounded-md border border-gold/40 bg-gold/[0.08] text-gold-dark px-3.5 py-2 leading-relaxed">
        A <strong>skeleton, not an email.</strong> Fill every <code className="text-gold-darker">[bracketed prompt]</code> in your own
      ...
      <div className="mt-4 rounded-md border border-gold/25 bg-gold/[0.04] px-3 py-2">
        <p className="text-[11px] uppercase tracking-[0.12em] text-gold-dark font-medium">
          ★ Your starred research ({starred.length}) — what each bracket reacts to
        </p>
        <div className="mt-1 divide-y divide-gold/15">
          {starred.map((f) => {
            ...
            return (
              <div key={key}>
                <button
                  ...
                  className="w-full flex items-start gap-2 py-1.5 text-left text-[13px] text-ink-2 hover:text-ink"
                >
                  <span className="text-gold mt-0.5">{open ? '▾' : '▸'}</span>
                  <span className={open ? 'font-medium' : ''}>{findingLabel(f)}</span>
                </button>
                {open && <p className="pb-2 pl-5 text-[12.5px] text-muted leading-relaxed">{f.content}</p>}
              </div>
            )
          })}
        </div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        <span className="text-xs text-muted-2 uppercase tracking-[0.1em]">Your ask</span>
        {ASKS.map((a) => (
      ...
      <div className="mt-4">
        <label className="text-xs text-muted-2 uppercase tracking-[0.1em]">Subject</label>
        <div className="mt-1 flex gap-2">
          <input
            value={subject}
            onChange={(e) => setSubject(e.target.value)}
            className="flex-1 min-w-0 px-3 py-2 bg-surface/70 border border-border-2 rounded-md text-ink text-base sm:text-sm focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent"
          />
          <button
            onClick={() => copyText('subject', subject)}
            className="shrink-0 px-3 py-2 text-[15px] border border-border-2 rounded-md text-accent hover:border-accent transition-colors"
          >
            {copied === 'subject' ? '✓' : 'Copy'}
          </button>
        </div>
      ...
      <div className="mt-4">
        <label className="text-xs text-muted-2 uppercase tracking-[0.1em]">Email</label>
        <textarea
          value={body}
          onChange={(e) => setBody(e.target.value)}
          className="mt-1 w-full min-h-[420px] lg:min-h-[58vh] px-3.5 py-3 bg-surface/70 border border-border-2 rounded-md text-ink text-base sm:text-sm font-mono leading-relaxed focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent resize-y"
        />
      </div>
      <div className="mt-2.5 flex flex-wrap items-center gap-3">
        ...
        <button
          onClick={regenerate}
          className="px-3.5 py-2 sm:py-1.5 text-[15px] border border-border-2 rounded-md text-accent hover:border-accent transition-colors"
          title="Rebuild subject + email from your current ask and starred research (replaces your edits)"
        >
          ↻ Regenerate
        </button>
        <span className="text-[13px] text-muted-2">Edits save automatically.</span>
      </div>
        ...
        {showExamples && (
          <div className="mt-4">
            <p className="text-[13px] text-muted leading-relaxed mb-4">
              ...
              Study <span className="font-medium text-ink">why</span> each highlighted line works, then write your own from
      ...
      <div className="mt-6">
        ...
        {showMore && (
          <div className="mt-3 space-y-3">
            {unstarred.length === 0 && <p className="text-xs text-muted-2">You&rsquo;ve starred everything from this lab.</p>}
            {unstarred.map((f) => (
