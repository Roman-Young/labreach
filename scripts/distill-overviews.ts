...
// text for the subagent to judge. Cap generously; ALWAYS keep the longest survivor even if it alone
// exceeds the cap (the fix for a research paragraph longer than the budget being dropped entirely).
export function cleanBio(bio: string): string {
  ...
  const byLen = [...paras].sort((a, b) => b.length - a.length)
  const primary = byLen[0]
  let out = primary.slice(0, 3200)
  let used = out.length
  for (const p of paras) { if (p === primary) continue; if (used + p.length > 3600) continue; out += '\n\n' + p; used += p.length }
  return out
}
